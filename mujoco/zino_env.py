...
class ZinoEnv(gym.Env):
    ...
    def __init__(self, xml_file_path="zino.xml", phase="wheels_only", render_mode=None, **kwargs):
        super().__init__()
        self.phase = phase
    ...
    def step(self, action):
        ...
        if np.random.rand() < 0.01:  # 3% chance per step
            kick = np.random.choice([-1.0, 1.0]) * np.random.uniform(10.0, 25.0)
